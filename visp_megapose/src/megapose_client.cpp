...
enum DetectionMethod { UNKNOWN, CLICK, DNN };

std::map<std::string, DetectionMethod> stringToDetectionMethod = {{"UNKNOWN", UNKNOWN}, {"CLICK", CLICK}, {"DNN", DNN}};
...
void MegaPoseClient::broadcastTransform(const geometry_msgs::msg::Transform &transform,
                                        const std::string &child_frame_id, const std::string &camera_tf)
{
...
void MegaPoseClient::spin()
{
  ...
  if (getDetectionMethodFromString(detectorMethod) == DNN) {
    ...
    vpDetectorDNNOpenCV::NetConfig netConfig(detectorConfidenceThreshold, detectorNmsThreshold, labels,
                                             cv::Size(640, 640), detectorFilterThreshold);
    // vpDetectorDNNOpenCV dnn(netConfig, detectorType);  // I don't know why this doesn't work. If I use this
    // it will cause the error "Cuda and/or GPU driver might not be correctly installed. Setting preferable backend to
    // CPU and trying again. terminate called after throwing an instance of 'cv::Exception"
    vpDetectorDNNOpenCV dnn_;
  ...
  while (!initial_pose_client->wait_for_service(1s) && !track_pose_client->wait_for_service(1s) &&
    ...
    ((__cplusplus >= 201703L) || (defined(_MSVC_LANG) && (_MSVC_LANG >= 201703L)))
      ...
      if (detection && init_request_done_) {
        ...
        auto result_future = initial_pose_client->async_send_request(
            ...
            std::bind(&MegaPoseClient::initial_pose_service_response_callback, this, std::placeholders::_1));
      ...
      if (track_request_done_) {
        ...
        auto result_future = track_pose_client->async_send_request(
            track_pose_request,
            std::bind(&MegaPoseClient::track_pose_service_response_callback, this, std::placeholders::_1));
...
void MegaPoseClient::initial_pose_service_response_callback(
    rclcpp::Client<visp_megapose::srv::Init>::SharedFuture future)
{
  init_request_done_ = true;
...
void MegaPoseClient::track_pose_service_response_callback(
    rclcpp::Client<visp_megapose::srv::Track>::SharedFuture future)
{
  track_request_done_ = true;
  transform_ = future.get()->pose;
  confidence_ = future.get()->confidence;
